...
<form action="${pageContext.request.contextPath}/UpdateProductController" method="post" enctype="multipart/form-data">
				...
				<table>
					<tr>
						...
						<td><input type="text" name="name" id="name" value = "<%= bean.getNome()%>" placeholder = "<%= bean.getNome()%>"></td>
					</tr>
					<tr>
						<td><label for="description">Descrizione:</label></td>
						<td><textarea name="description" id="description" rows="3" value = "<%= bean.getDescrizione()%>" placeholder = "<%= bean.getDescrizione()%>"></textarea></td>
					</tr>
					<tr>
						<td><label for="price">Prezzo Base:</label></td>
						<td><input type="number" name="price" id="price" min="0" step="0.01" value = <%= bean.getPrezzoBase()%>></td>
					</tr>
					<tr>
						<td><label for="quantity">Quantità:</label></td>
						<td><input type="number" name="quantity" id="quantity" min="0" value = <%= bean.getQuantita()%>></td>
					</tr>
					<tr>
						<td><label for="image">Immagine:</label></td>
						<td><input type="file" name="image" id="image"></td>
						<input type="hidden" name="currentImage" id = "currentImage" value="<%= bean.getImmagine() %>" placeholder="<%= bean.getImmagine() %>>">
					</tr>
					<tr>
